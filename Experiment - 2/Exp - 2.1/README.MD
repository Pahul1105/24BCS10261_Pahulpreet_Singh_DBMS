````markdown
# UNION

## Question

Till now, we have joined tables by columns. Now let's join the data by rows.

We use the concept of **UNION** to do that.

**UNION** helps us to place a table right on top of another table.

There are a set of criteria to be followed while appending the tables:

- The number of columns of the tables should be the same.
- The data type of the columns should be in the same order as that of the first table.

Below is the query to combine tables `Mfg_Ind` and `Mfg_Int`:

```sql
SELECT *
FROM Mfg_Ind
UNION
SELECT *
FROM Mfg_Int;
```

Below are the tables **Mfg_Ind** and **Mfg_Int**.

### Table: Mfg_Ind

| mfg_id | company |
|--------:|---------|
| 1001 | Mahindra |
| 1002 | Tata |

### Table: Mfg_Int

| Id | cmp_name |
|---:|----------|
| 5 | Hyundai |
| 6 | Suzuki |

After executing the above query, we get the table below:

| mfg_id | company |
|--------:|---------|
| 1001 | Mahindra |
| 1002 | Tata |
| 5 | Hyundai |
| 6 | Suzuki |

### Task

Write a query using **UNION** to stack the table **`Arts`** over **`Science`** and output the final table.

> **Note:** The `UNION` statement removes duplicate rows in the new table formed.

---

# Code

```sql
SELECT *
FROM Arts
UNION
SELECT *
FROM Science;
```

---

# Output

<p align="center">
  <img src="union.png" alt="UNION Output" width="700">
</p>
````

